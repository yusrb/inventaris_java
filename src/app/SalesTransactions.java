...
public class SalesTransactions extends javax.swing.JFrame {
    ...
    public SalesTransactions(String username, String level) {
        initComponents();
        
        javax.swing.table.JTableHeader header = tblTampilSalesTransactions.getTableHeader();
    ...
    public void Fetch() {
        try {
            pst = conn.prepareStatement("SELECT id, tanggal, total_item, total_harga, total_bayar, kasir FROM transaksi ORDER BY tanggal DESC");
            rslt = pst.executeQuery();

            String[] columnNames = { "ID", "Tanggal", "Total Item", "Total Harga", "Total Bayar", "Kasir" };
            ...
            DefaultTableModel model = new DefaultTableModel(columnNames, 0) {
                ...
                public Class<?> getColumnClass(int columnIndex) {
                    switch (columnIndex) {
                        ...
                        case 1: return String.class;     // Tanggal as String
                        case 2: return Integer.class;    // Total Item
                        case 3: return Integer.class;    // Total Harga
                        case 4: return Integer.class;    // Total Bayar
                        case 5: return String.class;     // Kasir
            ...
            while (rslt.next()) {
                ...
                row.add(rslt.getTimestamp("tanggal").toString());
                row.add(rslt.getInt("total_item"));
                row.add(rslt.getInt("total_harga"));
                row.add(rslt.getInt("total_bayar"));
                row.add(rslt.getString("kasir"));
                model.addRow(row);
            }

            tblTampilSalesTransactions.setModel(model);
            TableRowSorter<TableModel> sorter = new TableRowSorter<>(model);
            tblTampilSalesTransactions.setRowSorter(sorter);
            ...
            tblTampilSalesTransactions.getColumnModel().getColumn(0).setCellRenderer(leftRenderer); // ID
            tblTampilSalesTransactions.getColumnModel().getColumn(1).setCellRenderer(leftRenderer);  // Tanggal
            tblTampilSalesTransactions.getColumnModel().getColumn(2).setCellRenderer(leftRenderer); // Total Item
            tblTampilSalesTransactions.getColumnModel().getColumn(3).setCellRenderer(leftRenderer); // Total Harga
            tblTampilSalesTransactions.getColumnModel().getColumn(4).setCellRenderer(leftRenderer); // Total Bayar
            tblTampilSalesTransactions.getColumnModel().getColumn(5).setCellRenderer(leftRenderer);  // Kasir


        } catch (SQLException ex) {
            Logger.getLogger(SalesTransactions.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void batasiAkses() {
        switch (levelForPage.toLowerCase()) {
            ...
            case "petugas kasir":
                ...
                btnSalesTransactions.setVisible(false);
    ...
    private void initComponents() {
        ...
        tblTampilSalesTransactions = new javax.swing.JTable();
    ...
    tblTampilSalesTransactions.setModel(new javax.swing.table.DefaultTableModel(
        new Object [][] {
        ...
        new String [] {
            "id", "tanggal", "total_item", "total_harga", "total_bayar", "kasir"
        }
    ) {
    ...
    jScrollPane1.setViewportView(tblTampilSalesTransactions);

    jLabel3.setFont(new java.awt.Font("Palatino Linotype", 1, 18)); // NOI18N
    jLabel3.setText("Sales Transactions");
    ...
    private void btnDetailActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDetailActionPerformed
        ...
        int pilihBaris = tblTampilSalesTransactions.getSelectedRow();

        if (pilihBaris == -1) {
            JOptionPane.showMessageDialog(this, "Transaksi Tidak Ditemukan\nPilih Transaksi Dulu", "Transaksi Tidak Ditemukan", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int transaksiId = (int) tblTampilSalesTransactions.getValueAt(pilihBaris, 0);

        DetailSalesTransactions detail_page = new DetailSalesTransactions(usernameForPage, levelForPage, transaksiId);
    ...
    private void btnSearchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSearchActionPerformed
        ...
        try {
            String sql = "SELECT id, tanggal, total_item, total_harga, total_bayar, kasir FROM transaksi " +
                         "WHERE kasir LIKE ? OR DATE_FORMAT(tanggal, '%Y-%m-%d %H:%i:%s') LIKE ? " +
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new SalesTransactions("Username", "").setVisible(true);
            }
        });
    }
    ...
    private javax.swing.JTable tblTampilSalesTransactions;
